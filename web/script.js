...
function initializeDataTable() {
  dataTable = $('#dataTable').DataTable({
    responsive: true,
    columns: [{ data: 'date' }, { data: 'ct_num' }, { data: 'amount' }, { data: 'detail' }],
    columnDefs: [
      {
        targets: 2,
        render: function (data, type, row) {
          if (type === 'display') {
            ...
            if (data == $('#searchInput').val())
              return `<span class="bg-yellow-300 text-slate-800">${amount}</span>`
      ...
      { className: 'border-b border-slate-200 text-md', targets: '_all' },
    ],
        ...
        .map((item) => {
          return {
            ...
            detail: item.detail.replace(
              regex,
              '<span class="bg-yellow-300 text-slate-800">$1</span>'
            ),
            ct_num: item.ct_num.replace(
              regex,
              '<span class="bg-yellow-300 text-slate-800">$1</span>'
            ),
            amount: item.amount,
